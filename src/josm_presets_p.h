/*
 * SPDX-FileCopyrightText: 2016 Rolf Eike Beer <eike@sf-mail.de>
 *
...
public:
  ...
  attach_key *attach(preset_attach_context &attctx, const std::string &preset) const override;
...
public:
  ...
  attach_key *attach(preset_attach_context &attctx, const std::string &) const override;
  unsigned int rows() const override {
    return 1;
  }
};
...
class presets_element_checkbox : public presets_element_t {
  static bool widgetValue(attach_key *akey);
protected:
  bool matchValue(const std::string &val) const override;
public:
...
public:
  ...
  std::set<std::string> roles(const relation_t *relation, const object_t &obj) const override;
