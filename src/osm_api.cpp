...
void osm_upload_context_t::upload(const osm_t::dirty_t &dirty)
{
  appendf(nullptr, _("Log generated by %s v%s using API 0.6\n"), PACKAGE, VERSION);
  appendf(nullptr, _("User comment: %s\n"), comment.c_str());

  settings_t::ref settings = settings_t::instance();

  if(api_adjust(project->rserver)) {
    appendf(nullptr, _("Server URL adjusted to %s\n"), project->rserver.c_str());
