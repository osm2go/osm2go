...
static GtkWidget *scrolled_window_new(GtkPolicyType hpolicy, GtkShadowType shadow) {
  GtkScrolledWindow *scrolled_window = GTK_SCROLLED_WINDOW(gtk_scrolled_window_new(O2G_NULLPTR,
                                                                                   O2G_NULLPTR));
  gtk_scrolled_window_set_policy(scrolled_window, hpolicy, GTK_POLICY_AUTOMATIC);

  gtk_scrolled_window_set_shadow_type(scrolled_window, shadow);
...
void osm_upload(appdata_t &appdata, osm_t *osm, project_t *project) {
...
#else
                            scrolled_window_new(GTK_POLICY_NEVER, GTK_SHADOW_ETCHED_IN);
#endif
  ...
  GtkWidget *scrolled_window = scrolled_window_new(GTK_POLICY_AUTOMATIC, GTK_SHADOW_IN);
  ...
  gtk_box_pack_start_defaults(GTK_BOX(GTK_DIALOG(dialog.get())->vbox), scrolled_window);
  gtk_widget_show_all(dialog.get());
