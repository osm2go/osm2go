...
static void canvas_item_info_dechain(canvas_item_info_t *item_info) {
  ...
  canvas_item_info_t **itemP = &canvas->item_info;
  ...
  if(canvas->last_item_info == item_info)
    canvas->last_item_info = item_info->prev;
...
static void canvas_item_prepend(canvas_t *canvas, canvas_item_t *canvas_item, 
				canvas_item_info_t *item) {
  if(!canvas->item_info) {
    g_assert(!canvas->last_item_info);
    canvas->last_item_info = item;
  } else
    canvas->item_info->prev = item;

  item->next = canvas->item_info;
  canvas->item_info = item;  
  item->item = canvas_item;   /* reference to visual representation */
  item->canvas = canvas;

  canvas_item_destroy_connect(canvas_item, (GCallback)item_info_destroy, item);
}

static void canvas_item_append(canvas_t *canvas, canvas_item_t *canvas_item, 
			       canvas_item_info_t *item) {
  if(!canvas->last_item_info) {
    g_assert(!canvas->item_info);
    canvas->item_info = item;
  } else                        
    canvas->last_item_info->next = item;

  item->prev = canvas->last_item_info;
  canvas->last_item_info = item;  
  item->item = canvas_item;   /* reference to visual representation */
  item->canvas = canvas;
...
static canvas_item_info_t *canvas_item_get_info(canvas_t *canvas, 
						canvas_item_t *item) {
  /* search for item in chain */
  canvas_item_info_t *item_info = canvas->item_info;
  while(item_info && item_info->item != item) 
...
void canvas_item_info_push(canvas_t *canvas, canvas_item_t *item) {
  ...
  canvas_item_info_dechain(item_info);
  canvas_item_prepend(canvas, item_info->item, item_info);
}
...
void canvas_item_info_attach_circle(canvas_t *canvas, 
  ...
  canvas_item_prepend(canvas, canvas_item, item);
...
void canvas_item_info_attach_poly(canvas_t *canvas, canvas_item_t *canvas_item,
...
void canvas_item_info_get_at(canvas_t *canvas, gint x, gint y) {
  canvas_item_info_t *item = canvas->item_info;
  ...
  while(item) {
    ...
    case CANVAS_ITEM_CIRCLE: {
      gint xdist = abs(item->data.circle.center.x - x);
      gint ydist = abs(item->data.circle.center.y - y);
      
      if(xdist*xdist + ydist*ydist < 
	 item->data.circle.r*item->data.circle.r) {
	printf("circle item %p\n", item);
      }
    } break;
	...
	printf("bbox item %p, %d pts -> %d", item, 
	       item->data.poly.num_points, 
	       canvas_item_info_get_segment(item, x, y)); 

	if(item->data.poly.is_polygon) 
	  printf(" in_poly = %d",
		 inpoly(item->data.poly.points, 
