...
int main()
{
  ...
  assert_cmpstr(foo.toStdString(), "abc nkw def 1 ghi");

  foo = trstring("abc %1 def %1 ghi").arg("nkw").arg(1);
  assert_cmpstr(foo.toStdString(), _("abc nkw def nkw ghi"));

  foo = trstring("abc %1 def %2 ghi %3").arg("3.14").arg("nkw");
  assert_cmpstr(foo.toStdString(), "abc 3.14 def nkw ghi %3");

  foo = trstring("abc %1 def %2 ghi %3").arg(3).arg("nkw");
  assert_cmpstr(foo.toStdString(), "abc 3 def nkw ghi %3");
  ...
  assert_cmpstr(foo.toStdString(), "Retry 1/3 ");

  foo = trstring("Log generated by %1 v%2 using API 0.6\n").arg(PACKAGE).arg(VERSION);
  assert_cmpstr(foo.toStdString(), "Log generated by " PACKAGE " v" VERSION " using API 0.6\n");

  foo = trstring("%1: member in %2 '%3'").arg("way").arg("boundary").arg("Wennigsen");
  assert_cmpstr(foo.toStdString(), "way: member in boundary 'Wennigsen'");
